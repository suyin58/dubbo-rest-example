define(
		...
		[ "bui/menu", "bui/tab" ],
		function(l) {
			var k = BUI.app("PageUtil"), h = l("bui/menu"), m = l("bui/tab");
			var q = "dl-selected", c = "ks-hidden", z = "dl-last", y = "dl-hover", o = "nav-item", t = "dl-second-slib", f = "dl-tab-item", u = "dl-collapse", w = "dl-hide-current", v = "data-index", j = 145;
			function g(A) {
				window.topManager = A;
			}
			function b(A, B) {
			...
			function i(C, F, I, G, B) {
				var J = this, D = new h.SideMenu(I), E = new m.NavTab(F), K = $(I.render), A = K
						.next("." + t + "-con"), H = K.parents("." + f);
				if (A) {
					A.on("click", function() {
						H.toggleClass(u)
					});
			...
			function d() {
				var E = location.hash, D = 0, A = "", B = E.indexOf("/"), C = null;
				if (!E) {
					return null
				}
				if (B >= 0) {
			...
			BUI
					.augment(
							...
							{
								openPage : function(P) {
									var K = this, F = P.moduleId
											|| K._getCurrentModuleId(), D = P.id, M = P.title
											|| "\u65b0\u7684\u6807\u7b7e\u9875", C = P.href, O = P.isClose, E = P.closeable, B = P.reload, Q = P.search;
									var H = K._getModule(F);
									if (H) {
										var I = H.tab, G = H.menu, A = G
												.getItem(D), L = I
												.getActivedItem(), N = L ? L
												.get("id") : null, J = K
												._getModuleIndex(F);
								...
								operatePage : function(F, H, G, B) {
									...
									B = B || [];
									var A = this, C = A._getModule(F);
									if (C) {
										var D = C.tab, E = H ? D.getItemById(H)
												: D.getActivedItem();
										if (E && E[G]) {
											E[G].apply(E, B)
										}
								...
								_createModule : function(F) {
									var A = this, D = A._getModuleConfig(F), C = A
											.get("modules");
									if (!D) {
										return null
									}
									var F = D.id, B = "#J_" + F + "Tab", E = "#J_"
											+ F + "Tree";
									module = new i(F, {
										render : B,
										height : a() - 5
								...
								_initNavItems : function() {
									var J = this, D = J.get("navItems"), C = J
											.get("hideItmes");
									if (D.length === 0) {
										return
									}
									$('<div class="nav-item-mask"></div>')
											.appendTo($(D));
									var H = D.length, L = BUI.viewportWidth(), I = j, G = I
											* H, A = 0;
									if (G <= L) {
										return
									}
								...
								_initHideList : function() {
									var C = this, B = C.get("hideList"), A = C
											.get("hideItmes");
									if (B) {
										return
									}
									var E = '<ul class="dl-hide-list ks-hidden"></ul>', D = $(
											E).appendTo("body");
									B = D;
									$.each(A, function(F, G) {
										$(G).appendTo(B)
								...
								_initContents : function() {
									var A = this, C = A.get("modulesConfig"), B = A
											.get("navContent");
									B.children().remove();
									$
											.each(
													...
													function(E, F) {
														var G = F.id, D = [
																'<li class="dl-tab-item ks-hidden"><div class="dl-second-nav"><div class="dl-second-tree" id="J_',
																G,
																'Tree"></div><div class="',
																t,
																'-con"><div class="',
																t,
																'"></div></div></div><div class="dl-inner-tab" id="J_',
																G,
																'Tab"></div></li>' ]
																.join("");
														new $(D).appendTo(B)
								...
								_initNavListEvent : function() {
									var B = this, A = B.get("hideList"), C = B
											.get("navList");
									C
											.on(
													"mouseover",
													function(F) {
														var E = p(F.target), D = $(E), G = null;
														if (D && D.hasClass(z)
																&& A) {
															G = D.offset();
															G.top += 37;
								...
								_initLocation : function() {
									var A = this, E = d();
									if (E) {
										var C = E.pageId, F = E.search, D = A
												._getModuleIndex(E.moduleId);
										A._setModuleSelected(D);
										A._setPageSelected(D, C, true, F)
									} else {
										var B = A.get("currentModelIndex"), G = A
												._getModuleId(B);
										if (B == null) {
								...
								_getModuleIndex : function(C) {
									var B = this, A = 0;
									$.each(B.get("modulesConfig"), function(E,
											D) {
										if (D.id === C) {
											A = E;
											return false
								...
								_getCurrentPageId : function() {
									var A = this, E = A._getCurrentModuleId(), C = A
											._getModule(E), B = "";
									if (C) {
								...
								_setLastItem : function(E) {
									var A = this, B = A.get("lastShowItem");
									if (B === E) {
										return
									}
									var C = null, D = $(B);
									...
									if (!C) {
										C = $('<span class="icon icon-white  icon-caret-down '
												+ w + '">&nbsp;&nbsp;</span>')
									}
									C.appendTo(itemEl
											.children(".nav-item-inner"));
									A.set("lastShowItem", E)
								},
								_setModuleSelected : function(G, F) {
									var I = this, E = I.get("navItems"), C = I
											.get("navTabs"), H = I
											.get("currentModelIndex");
									if (H !== G) {
										var A = I._getModuleId(G), B = null, D = I
												.get("lastShowItem"), J = true;
								...
								_setPageSelected : function(F, G, H, L) {
									var I = this, C = I._getModuleId(F) || F, D = I
											._getModule(C);
									if (D && G) {
										D.menu.setSelectedByField(G);
										var K = D.menu.getSelected(), E = D.tab, B = "", A = -1;
										if (K && K.get("id") === G) {
											B = K.get("href");
											B = L ? (b(B, L)) : B;
											D.tab.addTab({
								...
								_setSelectHideItem : function(E) {
									var C = this, B = C.get("hideList"), A = C
											.get("hideItmes"), F = null, D = null, H = null, G = null;
									...
									if (F !== D) {
										...
										if (!G) {
											G = new Node(
													'<span class="dl-hide-current">&nbsp;&nbsp;</span>')
										}
										H = $(D);
										G.appendTo(H
												.children(".nav-item-inner"));
