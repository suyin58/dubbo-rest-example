 function createDetailHtml(title,arrays,detailDivId){
		$('#'+detailDivId).html('<h1 style="margin-bottom: 50px;margin-left: 30%;">'+title+'</h1>');
		...
		if(arrays && arrays.length > 0){
           ...
           for(var i in arrays){
        	   ...
        	   if(arrays[i]['groupId'] || arrays[i]['groupId'] == 0){
        		   ...
        		   colums = checkCols(detailDivId,arrays[i],colums);
                   if(colums % 3 == 0){
                     	if(colums == 3){
                            $('#'+detailDivId).children('div').wrapAll("<div class='row detail-row'> </div>");
	   	                }else{
	   	                   $('#'+detailDivId+' div:last').prevUntil('.detail-row').andSelf().wrapAll("<div class='row detail-row'> </div>");
	   	                }
                   }
        	   }
           }
           ...
           if(commonData){//普通数据
        	   $('#'+detailDivId).children('div:not(".detail-row")').wrapAll("<div class='row detail-row'> </div>");
           }else{
        	   //有groupId的数据 
 ...
 function createPanel(id){
	 $('#'+detailDivId).append('<div id="'+id+'" class="panel"></div>');
 }
 
function createTablePanel(title){
	$('#'+detailDivId).append('<div class="panel table" id="table"><div class="panel-header clearfix"><h3 class="pull-left">'+title+'</h3> </div><div class="row detail-row"><table class="bui-grid-table" cellspacing="0" cellpadding="0"  style="text-align : center;"></table></div></div>');
}
